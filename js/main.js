...
// ==========================
// LIGHTBOX (réutilisable — projet pages)
// ==========================
(function () {
  ...
  function openLightbox(trigger) {
    ...
    const type    = trigger.dataset.lbType    || "img";
    const src     = trigger.dataset.lbSrc     || "";
    const srcMp4  = trigger.dataset.lbSrcMp4  || "";
    const srcType = trigger.dataset.lbSrcType || "video/mp4";
    const alt     = trigger.dataset.lbAlt     || "";

    // Réinitialise les deux médias
    lbImg.classList.remove("lb-active");
    lbVideo.classList.remove("lb-active");
    lbImg.src = "";

    if (type === "video") {
      // Reconstruit les sources pour la compatibilité navigateur
      lbVideo.innerHTML = "";
      if (srcMp4) {
      ...
      lbVideo.muted = true;
      lbVideo.load();
      lbVideo.classList.add("lb-active");
      // Autoplay muet dès que le flux est prêt — l'utilisateur peut activer le son via les contrôles
      lbVideo.addEventListener("canplay", function onCanPlay() {
  ...
  function closeLightbox() {
    ...
    // Stoppe la vidéo immédiatement
    lbVideo.pause();

    // Retour du focus sur l'élément d'origine
    if (lastFocused) {
      lastFocused.focus();
      lastFocused = null;
    }
  ...
  // Câblage de tous les déclencheurs [data-lb-src]
  document.querySelectorAll("[data-lb-src]").forEach((trigger) => {
